...
describe('ThingRepository', () => {
  ...
  it('creates a thing', async () => {
    const thing = new Thing('thing-1', 'Alpha', new Date('2026-01-01T00:00:00.000Z'));
    await repository.save(thing);

    const found = await repository.findById('thing-1');
    expect(found?.toPrimitives()).toEqual(thing.toPrimitives());
  });

  it('lists things', async () => {
    await repository.save(new Thing('thing-1', 'Alpha', new Date('2026-01-01T00:00:00.000Z')));
    await repository.save(new Thing('thing-2', 'Beta', new Date('2026-01-02T00:00:00.000Z')));

    const all = await repository.findAll();
    expect(all).toHaveLength(2);
    expect(all[0].id).toBe('thing-1');
    expect(all[1].id).toBe('thing-2');
  });
  ...
  it('deletes a thing', async () => {
    ...
    const deleted = await repository.delete('thing-1');
    expect(deleted).toBe(true);

    const afterDelete = await repository.findById('thing-1');
